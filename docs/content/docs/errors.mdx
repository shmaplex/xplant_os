...
| `409` | `SOP_RUN_CLOSED` | The run has already ended: its status is one of `completed`, `failed`, `cancelled`, `archived`. Nothing changes. Get the run to see how it ended. Or: The run has ended: its status is one of `completed`, `failed`, `cancelled`, `archived`. An ended run's record is final and takes no more evidence. |
