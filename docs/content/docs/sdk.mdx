...
These docs match **@shmaplex/xplant-sdk 0.7.0**. The SDK runs on **Node.js 18+** and any runtime with standard `fetch`: Deno, Bun, edge and serverless functions. **Call the API from your server, not a web page.** The API doesn't send CORS headers, and keys don't belong in front-end code.
...
`client.rateLimit` is the request budget the latest response reported, from its [rate-limit headers](/docs/rate-limits#watching-your-budget): `{ limit, remaining, reset, observedAt }`, or `null`.
...
- `limit` and `remaining` are for whichever budget has fewer requests left: the key's (or device token's) 1,000, or the workspace's 3,000.
- Every success and every `429` updates it; other failures leave it as it was.
- It's `null` before the first response, and after a success that carried no budget because the API couldn't read it. Treat `null` as unknown, not unlimited.
- The [sensor readings budget](/docs/rate-limits#sensor-readings-have-their-own-budget) isn't included.
...
| `client.sopRuns` | `start`, `get`, `listEvents`, `recordStepEvent`, `recordMeasurement`, `complete` |
