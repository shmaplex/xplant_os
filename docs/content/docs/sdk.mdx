...
The SDK runs on **Node.js 18+** and any runtime with standard `fetch`: Deno, Bun, edge and serverless functions. **Call the API from your server, not a web page.** The API doesn't send CORS headers, and keys don't belong in front-end code.
...
SDK 0.5.0 adds methods for the newer endpoints: `plants.create` / `update`, `explants.create` / `update`, `contaminations`, `comments`, `assets`, `mediaRecipes`, `equipment.list` / `get` / `listEvents`, `pricing` and `commerce`. Until 0.5.0 is on npm, those endpoints' reference pages show a plain `fetch` example.
