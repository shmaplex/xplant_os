...
The base URL itself answers `GET` without a key: it returns the API's name, version, current release and links to these docs, which makes it a quick check that you can reach the API. (Opened in a browser, it sends you here.)
...
Successful responses also carry `X-RateLimit-Limit`, `X-RateLimit-Remaining` and `X-RateLimit-Reset`: how much of your request budget is left this minute. See [Rate limits](/docs/rate-limits#watching-your-budget).
