...
```js tab="JavaScript" tab-group="lang"
const closed = await client.sopRuns.complete(
  run.id,
  { outcome: "completed", notes: "All vessels labelled and moved to shelf 3." },
  { idempotencyKey: "station-3-wk38-complete" },
);
console.log(closed.status, closed.completedAt);
```
...
In the SDK, `sopRuns.get()` returns the whole trail in `run.events`, however long it is. For a very long run, `sopRuns.listEvents()` hands it to you a page at a time instead:

```js
const run = await client.sopRuns.get(runId);
console.log(`${run.events.length} pieces of evidence`);
...
Over plain HTTP, [Get an SOP run](/docs/api/sops/get-sop-run) returns the run, its steps and the first 50 pieces of evidence. If there are more, `meta.events_next_cursor` isn't `null`. Pass it as `cursor` to [List a run's evidence](/docs/api/sops/list-sop-run-events), and keep following `meta.next_cursor` until it's `null`:
