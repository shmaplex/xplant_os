...
A gateway that reads several probes is usually one device posting several reading types. A device has **one channel per reading type**: readings have no probe field, so two probes of the same type on one device would collide. Register the second one as its own device. If you'd rather see each probe as its own device, register each one and give the gateway a token per device.
...
Readings come **newest first**. Filter by `device_id`, `room_id` and `type`, and set the window with `since` and `until` (both inclusive). A page holds 100 readings by default and up to 1,000; follow `meta.next_cursor` for the rest, as on every [list](/docs/pagination).
