...
`transfer_date` is a calendar day, so send the day itself, such as `2026-09-24`. A full timestamp is accepted too, but its day is read in UTC, so `2026-09-26T08:00:00+09:00` lands on 25 September. Transfers come back with `transfer_date` as a timestamp; take its first ten characters as the day rather than converting it to your time zone, which can shift it by a day.
