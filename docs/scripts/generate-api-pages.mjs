 ...
 * spec (openapi/openapi.json), the scope catalogue (openapi/scopes.json) and
 * the examples in openapi/overlay.mjs.
 *
...
/** The spec's own catalogue once it carries `x-scopes`; until then the vendored copy. */
const scopeCatalogue =
  spec["x-scopes"] ?? JSON.parse(readFileSync(path.join(root, "openapi/scopes.json"), "utf8")).scopes;
...
function renderOperation(o) {
  ...
  if (o.extra.notes) lines.push(o.extra.notes, "");
...
// Scopes page
{
  ...
  for (const s of byScope.keys()) if (!known.has(s)) errors.push(`scope "${s}" is used by the spec but missing from openapi/scopes.json`);
  ...
  const lines = [
    ...
    "---",
    "",
    "{/* Generated by scripts/generate-api-pages.mjs from openapi/scopes.json and openapi/openapi.json. Do not edit by hand. */}",
    "",
